...
- [ ] **16 GB Mac 真实 dogfood 验证**（v0.6.3 #R6 risk）：本地 refiner baseline 是从 24 GB 外推的；找台真 16 GB Mac 跑一周本地 refine + 收 swap / refine latency 数据，决定 mid tier 默认是否要改，或调整警告文案。
- [ ] ~~**`#33` dogfood live mode 5+ 天信号采集**~~ — 已是长期累积任务，留 task 系统跟踪，不入 backlog。

## v0.7.0 close 迁入

- [ ] **#R10** Streaming refine integration tests —— Esc / focus loss / cloud↔local 主路径已人工覆盖；剩这几条 edge case 没自动覆盖：
  - mid-stream cloud↔local 切换（用户在流式中切 Settings → Local refiner toggle 的行为）
  - 网络断开（cloud SSE 中途 connection reset）
  - mlx OOM / model load 失败（streamResponse 抛错的传播）
  - 用户在流式期间手动键入（pasteboard 抢占 / 焦点状态）
  
  写 mocks 1-2 天，post-ship 增量加更划算。
- [ ] **#R11** 16 GB Mac dogfood —— 跟 v0.6.3 那条合并；v0.7.0 新引入 `LocalLiveSegmentSession` 跨段 ChatSession + KV cache 累积，可能比单次 refine 更早触发 swap，需要观察。
- [ ] **`replaceLastInjection` IME bypass**：live-mode cloud 的 session-end batch refine + Cmd+Z×N replace 路径在 CJK 输入法下脆弱（IME 拦截 Cmd+Z 当 composition undo）。raw-first 已砍所以不影响 one-shot；live 云端用户少所以延后。修法跟 `TextInjector.inject` 一样的 ASCII bypass，~10 行。dogfood 报告 → 修。

## v0.7.3 close 迁入

- [ ] **#B4 Refiner cold-path 分析工具** —— 原 v0.7.3 计划的 `Scripts/analysis/refine_cold_path.py`（从 `refines.jsonl` 算 first-of-day latencyMs、idle-gap × latencyMs 散点）+ `docs/perf/refiner-baseline.md`（baseline 数据 + 结论）。pin 决策已通过 #B8a 即兴分析回答，这两个 artifact 仍有价值作为长期 perf 监控基础，不阻塞任何版本。`refines.jsonl` 现在已带 `mlxActiveMb/cacheMb/peakMb` 字段（v0.7.3 #B8a），脚本应该顺手覆盖 cacheLimit 长期回归监控。

## v0.8.x 候选派生

- [ ] **自动热词建议（v0.8.0 #B5 后置）**：扫 `refines.jsonl` 找反复 `(input_token, output_token)` substitution（例如 "kuwen" → "Qwen" 出现 ≥3 次），在 Settings → Glossary tab 弹 "Suggested from history" section，用户一键 accept / dismiss。**不**自动加（错加污染所有后续转录）。依赖：#B5 schema 拍板（决定建议条目要不要 per-app 维度）。挂在 v0.8.2 或后置 patch。
- [ ] **Refine drift rate 看板**（评估后**不做**）：从 refines.jsonl 算"refine 改字率"（input != output 占比 + 平均 edit distance）。价值低于实现成本——它不等于 ASR 错误率，只是 refine 触发频率，且 #E1/#E2（v0.8.1）落地后这个指标的解释力会进一步被稀释。留个 placeholder 以防将来想做。

## v0.8.0 close 迁入

- [ ] **#S1 dogfood ≥1 周后跑 `skip_gate_report.py`**：v0.8.0 ship 时只有 11 条 verify 数据，4 个 gate state 各 1-2 例。1 周后跑全量，对比 Python prototype 的 ~16% skip rate 预测，看 hotword guard FP 实际占比。如果 phonetic guard 的 `noop_rate > 30%`（refine 跑了但没改），考虑收紧 `PhoneticMatcher.defaultThreshold`（当前 0.3）。
- [ ] **v0.8.0 schema migration post-ship 验证**：v0.8.0 内 ContextProfile schema 翻了两次（snippet → whitelist → independent，均 pre-release）。旧 `profiles.json`（含 `systemPromptSnippet` 或 pre-release `dictionaryFilter`）首次启动应静默丢弃这些字段，profile 重置为 `includeGlobal=true` + 空 `entries`（= 用全局、无私有）。如果用户报 "profile 没了" → 检查 `~/Library/Application Support/VoiceTyping/profiles.json` 是否被重命名成 `profiles.corrupted-*.json`。无报告 = 验证通过，可关闭。
- [ ] **Refine prompt 改后 cleanup mode 行为稳定性验证**：v0.8.0 修了 `MUST NOT rewrite` ↔ `Rewrite these` 的内部矛盾。1 周后回看 `refines.jsonl` 里 `light` mode 的 output 长度分布有无意外漂移（是否变得更激进/保守）。最好的兜底是 v0.8.1 #E1 (refine CER eval) 落地后做正式回归。
