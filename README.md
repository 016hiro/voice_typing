...
- **Streaming refine** (default, v0.7.0): refine output streams into the focused app as the LLM generates it — both cloud and local backends — so characters arrive instead of a silent multi-second pause. Press Esc or switch apps to cancel mid-stream; Notion auto-degrades to one-shot paste.
