...
### Fixed
- **闲置后首次 ASR 卡顿（v0.6.0 起的存量 bug，v0.6.4/v0.7.1 两轮都没修干净）**：闲置 1-2h 后 macOS 把 MLX 权重压成冷页，下次按 Fn 要等 9-30s 解压；overnight 11h 后甚至 37-51s。v0.7.1 用 keep-alive timer 周期触摸权重的方法被 dogfood 数据证伪——timer tick 自身就跑 30-45s（silent buffer 没 EOS 信号，decoder 跑满整个 budget）。v0.7.2 改用 mlx-swift 的 `WiredMemoryTicket` 把权重页钉死在 GPU residency 集合里，compressor 看到就跳过。Dogfood 56 sessions 验证：≥5s 慢段比例 5.3% → **0%**，最差段 30-51s → **3.56s**，11.5h 通宵 idle 后第一段 1.08s
- **Live 模式偶发"录音转字 30+ 秒不出"**：dogfood 抓到一例首段 145ms 转完后第二段无声卡死的捕获。根因是 Silero VAD 和 Qwen ASR 在 MLX 内部共享同一把 lock，VAD 冷路径占住时整个推理链全等。修法：live 模式 VAD 切换到 CoreML 路径走 ANE+CPU 跟 MLX 分开；同时加被动 watchdog（疑似挂起 5s 后自动抓线程栈到 `~/Library/Application Support/VoiceTyping/hang-stacks/` 方便后续诊断）

### Removed
- **v0.6.4 引入的 ASR keep-alive timer 整段拆掉**：90s 周期 dummy transcribe 既保不了温（自身在 cold path 上）又增加电耗，且偶尔会跟用户 Fn↓ 抢 MLX 锁。`WiredMemoryTicket` pin 替代它。代码上是 -250 行净减

### Notes
- 内存占用：Qwen ASR 后端（默认）加载后会保留 ~1.5 GB unified memory 不被压缩，直到切换到 Whisper 后端或退出 app。这是为换冷启动延迟做的取舍，详见 [`docs/decisions/0002-pin-mlx-weights-not-keep-alive.md`](docs/decisions/0002-pin-mlx-weights-not-keep-alive.md)
- 完整开发记录：[`docs/devlog/v0.7.2.md`](docs/devlog/v0.7.2.md)

## v0.7.1 — 2026-05-02

### Fixed
- **闲置后首次 ASR 卡顿**（v0.6.4 修的同一个 bug，v0.6.4 实际没修干净）：v0.6.4 ship 后 dogfood 数据显示 ≥5 s outlier 反而从 2.2% 升到 5.3%，根因是当时的 keep-alive timer 在 release 里被 macOS App Nap 节流，且没人能观测到（log 默认 off）。v0.7.1 加 `ProcessInfo.beginActivity(.userInitiated)` 抑制 App Nap、`NSWorkspace.didWakeNotification` 监听 wake-from-sleep 立即补 tick、timer callback 的 QoS 从 `.background`（最低，可被任意 defer）升到 `.utility`、所有 keep-alive 日志从 `.dev` 升 `.notice`（release 默认可见）— **这一轮事后被 v0.7.2 dogfood 证伪并整段移除，见 v0.7.2 entry**
